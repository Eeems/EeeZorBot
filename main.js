...
function exit() {
  for (var i in connections) {
    disp.log(
      "Disconnecting from " +
        connections[i].config.host +
        ":" +
        connections[i].config.port
    );
    connections[i].quit();
  }
  unloadScripts();
...
var irc = (global.irc = function(
  ...
  this.createSocket = function() {
    ...
    this.socket.on("error", function(e) {
      ...
      if (
        !this.parent.reconnecting &&
        ++this.parent.reconnect.attempts <=
          this.parent.config.reconnect_attempts
      ) {
        disp.alert(
        ...
        try {
          if (
            this.parent.config !== undefined &&
            this.config.parent.channels !== undefined
          ) {
  ...
  this.quit = function() {
    ...
    try {
      ...
      disp.alert(
        this.config.host + ":" + this.config.port + " already disconnected"
      );
    }
    for (i in connections) {
      if (connections[i] == this) {
        disp.log(
          "Disconnecting from " + this.config.host + ":" + this.config.port
        );
  ...
  this.reconnect = function() {
    ...
    try {
      ...
      this.socket.connect(this.config.port, this.config.host, function() {
        ...
        this.parent.send(
          "USER " +
            sanitize(this.config.username) +
            " localhost * " +
            sanitize(this.config.name)
        );
      });
        ...
        if (hooks[i].type == "send") {
          ...
          if (match) {
            try {
              ...
              disp.trace();
              disp.error(
                "caught error in script " + hooks[i].script + ": " + err
              );
            }
  ...
  this.socket.connect(this.config.port, host, function() {
    ...
    this.parent.send(
      "USER " + sanitize(username) + " localhost * " + sanitize(name)
    );
  });
  for (var i in hooks) {
    ...
    switch (op) {
      case "HOOKS":
        ...
        for (i in global.hooks) {
          h = global.hooks[i];
          disp.log(["Hook: " + i, " Type: " + h.type, " Script: " + h.script]);
        }
        break;
      case "SCRIPTS":
          ...
          for (i = 0; i < d_scripts.length; i++) {
            if (d_scripts[i].substr(-3) == ".js") {
              ...
              disp.log([
                "Name: " + d_scripts[i],
                " State: " +
                  (inArray(d_scripts[i], global.scripts.getAll())
                    ? "ENABLED"
                    : "DISABLED"),
                " Hooks: " + c,
                " Help Entries: " + h
      ...
      case "DISABLE-SCRIPTS":
        if (data.length >= 1) {
          for (i in data) {
            unloadScript(
              data[i].substr(-3) == ".js" ? data[i] : data[i] + ".js",
              true
            );
          }
        } else {
      ...
      case "ADD-SERVER":
        if (data.length >= 2) {
          ...
          servers.add(
            '{"host":"' + data[0] + '","port":"' + data[1] + '"' + nickSs + "}"
          );
        } else {
          disp.error(" not enough arguements");
        }
      ...
      case "MSG":
        if (data.length >= 3) {
          connections[data.splice(0, 1)].reply(
            data.splice(0, 1),
            data.join(" ")
          );
        } else {
...
with (api) {
  listen(/^PING :(.+)/i, function(match, data, replyTo, connection) {
    connection.send(
      "PONG :" +
        (typeof match[1] == "undefined" ? connection.config.host : match[1])
    );
    for (i = 0; i < hooks.length; i++) {
  ...
  listen(rCommand("help", true), function(match, data, replyTo, connection) {
    ...
    for (i in helpdb) {
      if (
        helpdb[i].name ==
        new RegExp("^:([^!]+).*" + config.prefix + "help (.*)$", "i").exec(
          data
        )[2]
      ) {
  ...
  regHelp(
    "help",
    "lists all commands, or displays the help on a specific command"
  );
  ...
  listen(
    /^:([^!]+).*:(End of \/MOTD command.|MOTD File is missing)\r/i,
  ...
  listen(rCommand("enable", true), function(match, data, replyTo, connection) {
    if (validUser(match[2], match[1]) && isOp(match[2])) {
      ...
      if (loadScript(script, true)) {
        ...
        connection.reply(
          replyTo,
          "Script " + script + " could not be enabled."
        );
      }
    }
  });
...
for (var i in srvs) {
  try {
    ...
    connections.push(
      new irc(s.host, s.port, config.nick, config.username, config.name, n, c)
    );
  } catch (e) {
