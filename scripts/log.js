...
if (!fs.exists("data/templates/chatlog.html")) {
  var template =
    '<html><head><title>{Title}</title></head><body>{Item(<a href="{Link}" name="{Anchor}">{timestamp}</a>{Text}<br/>)}<a name="end"></a></body></html>';
...
listen(/^.+/i, function(match, data, replyTo, connection) {
    ...
    try {
      ...
      if (nick.toLowerCase() == "omnomirc") {
        if ((data2 = /\([#OC]\)[\W0-9]*<(.+)>(.+)$/.exec(data)) !== null) {
          ...
          (data2 = /\([#OC]\)[\W0-9]* ?(.+) has left .+$/.exec(data)) !== null
        ) {
          save(
          ...
          (data2 = /\([#OC]\)[\W0-9]* ?(.+) has joined .+$/.exec(data)) !== null
        ) {
          save(
            connection.config.host +
              " " +
  ...
  if (
    replyTo == ". server ." &&
    new RegExp("^:(.+) 372 " + connection.config.nick + " :(.+)$").exec(
      data
    ) === null
  ) {
    save(p, {
...
function save(log, o) {
  ...
  switch (config.logtype) {
    ...
    default:
      ...
      fs
        .createWriteStream("data/logs/" + log + ".log", {
          flags: "a",
          encoding: "ascii"
        })
        .end(
  ...
  server = (global.logServer = http
    .createServer(function(req, res) {
      ...
      if (req.url == "/") {
        ...
        res.write(
          "<!doctype html><html><head><link rel='icon' type='image/x-icon' href='favicon.ico' /><title>Logs</title><script src='http://code.jquery.com/jquery-1.10.2.min.js'></script><script src='http://code.jquery.com/ui/1.10.3/jquery-ui.min.js'></script><link rel='stylesheet' href='http://code.jquery.com/ui/1.9.2/themes/base/jquery-ui.css' type='text/css'>"
        );
        res.write(
          "<script>$(function(){$('.accordion').accordion();$('.datepicker').datepicker({dateFormat:'D M dd yy',maxDate: new Date}).datepicker('setDate','0');$('.open').click(function(){location = '?server='+$(this).prev().prev().prev().val()+'&channel='+$(this).prev().prev().val().substr(1)+'&date='+$(this).prev().val();})})</script>"
        );
        ...
        var j,
          ...
          folder_size = function() {
            var size = function(file) {
                ...
                try {
                  size = fs.statSync(
                    "data/logs/" + log.name + "/" + channel + "/" + file
                  ).size;
                } catch (e) {
                  console.log(e);
                }
        ...
        for (i = 0; i < logs.length; i++) {
          ...
          res.write(
            "<h3>" +
              log.name +
              '</h3><div><input type="hidden" value="' +
              log.name +
              '"/><select>'
          );
          ...
          res.write(
            '</select><input class="datepicker"/><button class="open" value="Open">Open</button></div>'
          );
        }
            ...
            if (log.type == "json") {
              ...
              res.write(
                "<html><head><title>" +
                  ...
                  " " +
                  n +
                  '</title><script src="http://code.jquery.com/jquery-1.10.2.min.js"></script></head><body><a name="start"></a><h1>Server: ' +
                  log.server +
                  "</h1><h2>Channel: #" +
                  log.channel +
                  "</h2><h3>Date: " +
              ...
              res.write(
                '<button value="<- Back" onclick="location=window.location.protocol+\'//\'+window.location.host;"><- Back</button><button value="Bottom" onclick="location.hash=\'end\'">Bottom</button><br/>'
              );
              for (i in l) {
                try {
                  e = JSON.parse(l[i]);
                  var end = "";
                  e.msg = htmlEntities(typeof e.msg != "undefined" ? e.msg : "")
                    .replace(
                      /[\x02\x1f\x16\x0f]|\x03(\d{0,2}(?:,\d{0,2})?)/g,
                      function(m) {
                        var style = "",
                          colour,
                          background,
                        ...
                        switch (type) {
                          case "\x0f":
                            type = "\x03";
                            style =
                              "text-decoration:none;font-weight:normal;text-decoration:none;";
                            colour = 1; // black
                            background = 0; //white
                            break;
                          case "\x1f":
                        ...
                        if (type == "\x03") {
                          ...
                          return (
                            "<span style='color:" +
                            getColour(colour, "inherit") +
                            ";background-color:" +
                            getColour(background, "transparent") +
                  ...
                  td = new Date(e.date);
                  res.write(
                    "<div class='log-entry'><a href=\"?server=" +
                      log.server +
                      "&channel=" +
                      log.channel +
                      "&date=" +
                      ...
                      '">' +
                      "[" +
                      addZero(td.getUTCHours()) +
                      ":" +
                      addZero(td.getUTCMinutes()) +
                      ":" +
                      addZero(td.getUTCSeconds()) +
                      "]</a>" +
                      m +
                      "</div>"
                  );
                  ...
                  res.write(
                    "*Please contact the owner of this bot. The logs have invalid characters*<br/>"
                  );
                }
              }
              res.end(
                '<button value="<- Back" onclick="location=window.location.protocol+\'//\'+window.location.host;"><- Back</button><button value="Top" onclick="location.hash=\'start\'">Top</button><a name="end"></a>' +
                  "\n" +
                  '<script>$(".log-entry").each(function(){$(this).html($(this).html().replace(/(\\b(https?|ftps?|file|irc):\\/\\/[-A-Z0-9+&@#\\/%?=~_|!:,.;]*[-A-Z0-9+&@#\\/%=~_|])/ig,"<a href=' +
                  "'$1'" +
                  '>$1</a>"));});</script></body></html>'
              );
            }
            break;
          case "txt":
  ...
  if (e.code == "EADDRINUSE") {
    setTimeout(function() {
      if (count < Settings.check_timeout) {
        count++;
        disp.log(
          "Port " + Settings.port + " in use, reconnect attempt: " + count
        );
        try {
          server.close();
        } catch (e) {}
