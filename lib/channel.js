...
module.exports = function(server, name){
    if(!server){
        throw new Error('Argument "server" must be defined');
    }
    if(!server.send){
        throw new Error('Argument "server" must be an instance of Server. ' + (typeof server));
    }
    if(!name){
        throw new Error('Agument "name" must be defined');
    }
    var self = this,
        topic = null;
    /**
