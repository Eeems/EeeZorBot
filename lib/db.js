...
module.exports = {
	...
	multiQuery: function(scripts,callback){
		(function(self,scripts,callback){
			var rs = [],
				run = function(i){
					var a = [];
					...
					self.query.apply(self,a);
				};
			run(0);
		})(this,scripts,callback===undefined?function(){}:callback);
	},
	multiQuerySync: function(){
		...
		this.multiQuery.apply(this,args);
		while(sync){
			deasync.sleep(1);
		}
		return data;
	...
	hasIndexSync: function(table,name){
		var sync = true,
			data;
		this.hasIndex(table,name,function(v){
			data = v;
			sync = false;
		});
		while(sync){
	...
	createIndex: function(table,name,column,callback){
		callback = callback===undefined?function(){}:callback;
		var self = this;
		self.hasIndex(table,name,function(e){
			if(!e){
				self.query("CREATE INDEX "+pool.escapeId(name)+" ON "+pool.escapeId(table)+" ("+pool.escapeId(column)+")",function(e){
					if(e){
						throw e;
					}
	...
	createIndexSync: function(table,name,column){
		var sync = true,
			data;
		this.createIndex(table,name,column,function(){
			data = arguments;
			sync = false;
		});
		while(sync){
	...
	insert: function(table,columns,callback){
		var self = this;
		callback = callback===undefined?function(){}:callback;
		self.query("INSERT INTO "+pool.escapeId(table)+" SET ?",[columns],function(e,r,f){
			if(e){
				throw e;
			}
			callback.call(self,r.insertId);
		});
	},
	insertSync: function(table,columns){
		var sync = true,
			data;
		this.insert(table,columns,function(r){
			data = r;
			sync = false;
		});
		while(sync){
