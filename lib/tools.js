var path = require('path'),
	...
	watch_handles = {},
	watch_folder_handles = {},
	sub_handles = {},
	Prop = module.exports.Prop = function(props){
		for(var i in props){
			this[i] = props[i];
		}
		return this;
...
module.exports.extend(module.exports,{
	...
	folder: {
		watch: function(folder){
			...
			if(!watch_folder_handles[folder]){
				watch_folder_handles[folder] = {
					files: {},
					path: folder,
			...
			return watch_folder_handles[folder];
		},
		unwatch: function(folder){
			folder = path.normalize(folder);
			if(watch_folder_handles[folder]){
				var fh = watch_folder_handles[folder],
					f;
				while((f = fh.files.pop())){
					f.unwatch();
				}
				delete watch_folder_handles[folder];
			}
		}
	},
