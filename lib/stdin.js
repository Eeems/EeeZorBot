...
var api,
	...
	stdin = {
		...
		console: function(method){
			if(config.levels[method] && console[method] !== undefined){
...
stdin.add('help',function(argv){
	...
	.add('debug',function(){
		...
		global.users = new global.api.Users('test');
	},'Starts the debugger')
	.add('raw',function(argv){
		argv.shift();
		api.servers[argv.shift()].send(argv.join(' '));
		...
		if(server !== undefined){
			if(argv.length > 2){
				...
				for(i=0;i<server.users.length;i++){
					console.log('	'+server.users[i].nick);
				}
			}
		}else{
			console.log('Servers:');
			for(i=0;i<api.servers.length;i++){
				console.log('	'+i+') '+api.servers[i].config.nick+'@'+api.servers[i].config.host);
			}
		}
