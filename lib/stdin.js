	...
	.add('info',function(argv){
		...
		if(argv.length>1){
			switch(argv[1]){
				...
				break;
				case 'ws':
					var server = require('./api.js').websocket.servers[argv[2]];
					if(server){
				...
				break;
				case 'socket':
					var server = require('./api.js').socket.servers[argv[2]];
					if(server){
				...
				break;
				case 'memory':
					var mem = mem = process.memoryUsage();
