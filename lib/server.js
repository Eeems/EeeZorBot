...
module.exports = function(config){
	...
	self.on(new RegExp(':.+ 311 .+ (.+) (.+) (.+) * :(.+)','i'),function(m){
		// 1 - nick
		// 2 - username
		// 3 - host
		// 4 - realname
	...
	self.on(new RegExp(':.+ 319 .+ (.+) :(.+)','i'),function(m){
		// 1 - Nick
		// 2 - channels
	...
	//:my.server.name 324 nodebot #irp +mns
	self.on(new RegExp(':.+ 324 .+ (.+) \\+(.+)','i'),function(m){
		// 1 - channel
		// 2 - modes
		...
		for(i=0;i<modes.length;i++){
			channel.modes.push(modes[i]);
		}
	});
	self.on(new RegExp(':(.+)\\!(.+)@(.+) MODE (.+) (.+) ?(.+)?','i'),function(m){
			...
			if('+-'.indexOf(mode) !== -1){
				...
				if(argv[ii] !== undefined){
					...
					if(state == '+'){
						...
						mode = user.modes[channel.name].indexOf(mode);
						if(mode !== -1){
							user.modes[channel.name].splice(mode,1);
						}
					}
				}else{
					if(state == '+'){
						if(channel.modes.indexOf(mode) === -1){
							channel.modes.push(mode);
						}
					}else{
						mode = channel.modes.indexOf(mode);
						if(mode !== -1){
							channel.modes.splice(mode,1);
						}
					}
				}
				ii++;
