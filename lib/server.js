...
module.exports = function(config){
	...
	self.on(new RegExp(':.+ 311 .+ (\\S+) (.+) (.+) * :(.+)','i'),function(m){
		// 1 - nick
		// 2 - username
		// 3 - host
		// 4 - realname
	...
	self.on(new RegExp(':.+ 319 .+ (\\S+) :(.+)','i'),function(m){
		// 1 - Nick
		// 2 - channels
	...
	self.on(new RegExp(':.+ 324 .+ (\\S+) \\+(.+)','i'),function(m){
		// 1 - channel
		// 2 - modes
	...
	self.on(new RegExp(':.+ (367|348|346) \\S+ (\\S+) (\\S+) (\\S+) (\\d+)','i'),function(m){
		// 1 - code
		// 2 - channel
		// 3 - hostmask
		// 4 - user
	...
	//:my.server.name 332 nodebot #irp :testing
	self.on(new RegExp(':.+ (331|332) \\S+ (\\S+) :(.+)','i'),function(m){
		// 1 - code
		// 2 - channel
		// 3 - topic
	...
	self.on(new RegExp(':(\\S+)\\!(.+)@(.+) TOPIC (\\S+) :(.+)','i'),function(m){
		// 1 - nick
		// 2 - username
		// 3 - host
		// 4 - channel
	...
	self.on(new RegExp(':(.+)\\!(.+)@(.+) MODE (\\S+) (\\S+) ?(.+)?','i'),function(m){
