...
module.exports = function(config){
	...
	self.on('error',function(e){
		...
		// PRIVMSG
		.on(/^:([^ ]+)!([^ ]+)@([^ ]+)\sPRIVMSG\s(\#?\w+)\s:?([^\x01].+?[^\x01])$/i,function(m){
			// 1 - nick
			// 2 - username
			// 3 - host
			// 4 - channel
		...
		// ACTION
		.on(/^:([^ ]+)!([^ ]+)@([^ ]+)\sPRIVMSG\s(\#?\w+)\s:?\x01ACTION (.+)?\x01$/i,function(m){
			// 1 - nick
			// 2 - username
			// 3 - host
			// 4 - channel
