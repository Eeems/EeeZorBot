...
var fs = require('fs'),
    ...
    get_servers = function(callback){ // eslint-disable-line camelcase
        var ret = [];
                        ...
                        if(stats.isDirectory()){
                            ...
                            l_channels.forEach(function(l_channel){ // eslint-disable-line camelcase
                                if(l_channel.search(/^#[^ #]+$/) !== -1){
                                    channels.push({
                                        name: l_channel,
                                        path: 'data/logs/' + l_server + '/' + l_channel + '/', // eslint-disable-line camelcase
                                        type: 'channel',
                                        server: server
                                    });
                                }
...
stdin.add('convert', function(argv){
    get_servers(function(l_servers){ // eslint-disable-line camelcase
        var actions = {
            help: function(){
            ...
            ls: function(argv){
                ...
                if(argv.length === 0){
                    ...
                    l_servers[argv[0]].channels.forEach(function(l_channel){ // eslint-disable-line camelcase
            ...
            do: function(argv){
                if(converting){
                    ...
                    if(argv.length === 0){
                        ...
                        l_servers.forEach(function(l_server, i){ // eslint-disable-line camelcase
                            status.servers[0] = i;
                            convert(l_server);
                        });
...
script.unload = function(){
    convert = undefined;
    get_servers = undefined; // eslint-disable-line camelcase
    id = undefined;
